...
class threejsViewer {
    constructor(domElement) {
        ...
        this.renderVolume = function (volume, colormap, arg) {
            ...
            if (mesh == null) {
                //first time initial
                let geometry = new THREE.BoxGeometry(dims[0], dims[1], dims[2]/*長寬高*/)
...
*/            
                ...
                uniforms = mesh.material.uniforms
                uniforms['u_cmdata'].value = new THREE.DataTexture(colormap, 256, 1, THREE.RGBAFormat)
                //or

                uniforms['u_data'].value.image = { data: volume.alpha, width: dims[0], height: dims[1], depth: dims[2]}
                uniforms['u_data'].value.needUpdata = true
                uniforms['u_renderstyle'].value = arg.renderType
